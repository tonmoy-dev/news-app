...
export default function MostViewsNewsSlider({ news }) {
    ...
    return (
        <Slider {...settings2}>
            <div className="post_gallery_items pt-2">
                {
                    news?.slice(0, 7)?.map(post => (
                        <div className="gallery_item gallery_item-style-2" key={post?.id}>
                            <div className="gallery_item_thumb">
                                <Image
                                    className='w-[80px] h-[64px] object-cover'
                                    src={post?.thumbnail_img_small ? (`/assets/images/${post?.thumbnail_img_small}`) : "https://placehold.co/80x64.png"}
                                    alt={post?.title}
                                    width={80}
                                    height={64}
                    ...
                    news?.slice(0, 6)?.map(post => (
                        <div className="gallery_item gallery_item-style-2" key={post?.id}>
                            <div className="gallery_item_thumb">
                                <Image
                                    className='w-[80px] h-[64px] object-cover'
                                    src={post?.thumbnail_image_small ? (`/assets/images/${post?.thumbnail_image_small}`) : "https://placehold.co/80x64.png"}
                                    alt={post?.title}
                                    width={80}
                                    height={64}
                                // blurDataURL="data:..." automatically provided
