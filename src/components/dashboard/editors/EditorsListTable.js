...
const EditorsListTable = ({ editorsData }) => {
    const [reporters, setReporters] = useState(editorsData)
    ...
    async function updateEditorData(id, fieldName, value) {
        try {
    ...
    const handleToggle = (id, fieldName, value) => {
        ...
        toast.promise(
            updateEditorData(id, fieldName, fieldValue),
            {
                loading: 'Updating editor data...',
                success: <b>Editor data is updated!</b>,
        ...
        // action
        {
            ...
            render: (_, record) => (
                <div style={{ display: "flex", alignItems: "center", gap: "15px" }}>
                    {/* block/ unblock editor */}
                    <Space size="middle">
                        <Tooltip title={record?.status === "blocked" ? 'Unblock Reporter' : 'Block Editor'}>
                            <Switch
                    ...
                    {/* remove editor */}
                    <Space size="middle">
                        <Tooltip title={"Remove this editor"}>
                            <Button
                                danger
                                icon={<DeleteOutlined />}
                                onClick={() => {
...
export default EditorsListTable;